...
#include "../Utils/IPEncryption.hpp"
...
vector<IPXFeatureRecord *> IPXMapDataDBAdapter::getAllRecordsOnFloor(
		int floor) {
	FeatureVector resultVector;
	...
	ostringstream ostr;
	ostr
			<< "select GEOMETRY, GEO_ID, POI_ID, CATEGORY_ID, NAME, SYMBOL_ID, LAYER, LEVEL_MAX, LEVEL_MIN from MAPDATA where FLOOR_NUMBER = "
			<< floor;
	...
	int ret = sqlite3_prepare_v2(m_database, sql.c_str(), (int) sql.length(),
			&stmt, NULL);
	if (ret == SQLITE_OK) {
...
//			s.clear();
//			s.write((const char *) sqlite3_column_blob(stmt, 0),
//					sqlite3_column_bytes(stmt, 0));

			const char *encryptedGeometryBytes =
					(const char *) sqlite3_column_blob(stmt, 0);
			int encryptedGeometryByteLength = (int) sqlite3_column_bytes(stmt,
					0);
			char decryptedGeometryBytes[encryptedGeometryByteLength + 1];
			decryptBytes(encryptedGeometryBytes, decryptedGeometryBytes,
					encryptedGeometryByteLength);
			s.clear();
			s.write((const char *) decryptedGeometryBytes,
					encryptedGeometryByteLength);
