...
class IPLBLabelBorderCalculator {
	static final String TAG = IPLBLabelBorderCalculator.class.getSimpleName();

	private static final double DPI_SCALE = 1.0;

	private static final int FACILITY_LOGO_SIZE = 26;

	public static IPLBLabelBorder getFacilityLabelBorder(Point screenPoint) {
		...
		return new IPLBLabelBorder(xmin, xmax, ymax, ymin);
	}

	public static IPLBLabelBorder getTextLabelBorder(IPLBTextLabel textLabel,
			Point screenPoint) {
		double scale = DPI_SCALE;
		...
		return new IPLBLabelBorder(left, right, bottom, top);
	}
