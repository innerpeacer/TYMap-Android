...
class IPLabelBorderCalculator {
	static final String TAG = IPLabelBorderCalculator.class.getSimpleName();

	private static final double DPI_SCALE = 1.0;

	private static final int FACILITY_LOGO_SIZE = 26;

	public static IPLabelBorder getFacilityLabelBorder(Point screenPoint) {
		...
		return new IPLabelBorder(xmin, xmax, ymax, ymin);
	}

	public static IPLabelBorder getTextLabelBorder(IPTextLabel textLabel,
			Point screenPoint) {
		double scale = DPI_SCALE;
		...
		return new IPLabelBorder(left, right, bottom, top);
	}
