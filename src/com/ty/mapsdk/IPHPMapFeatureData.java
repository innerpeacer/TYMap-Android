...
class IPHPMapFeatureData {
	static final String TAG = IPHPMapFeatureData.class.getSimpleName();

	private TYBuilding currentBuilding;

	public IPHPMapFeatureData(TYBuilding building) {
	...
	Map<String, Graphic[]> getAllMapDataOnFloor(int floor) {
		...
		String dbPath = IPHPMapFileManager.getMapDataDBPath(currentBuilding);
	...
	static Geometry agsGeometryFromRecord(IPXFeatureRecord record) {
		return IPHPGeos2AgsConverter.agsGeometryFromGeosGeometry(record
				.getGeometry());
	}
}
